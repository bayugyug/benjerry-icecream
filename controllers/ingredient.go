...
func (api *ApiHandler) CreateIngredient(w http.ResponseWriter, r *http.Request) {
	...
	//chk
	if !data.SanityCheck(data, "ADD-INGREDIENT") {
	...
	render.JSON(w, r, APIResponse{
		Code:   http.StatusOK,
		Status: "Create Ingredients successful",
	})
...
func (api *ApiHandler) DeleteIngredient(w http.ResponseWriter, r *http.Request) {
	...
	data.ID = strings.TrimSpace(chi.URLParam(r, "id"))
	//chk
	if !data.SanityCheck(data, "DELETE-INGREDIENT") {
		utils.Dumper("MISSING_REQUIRED_PARAMS", data)
		//206
		api.ReplyErrContent(w, r, http.StatusPartialContent, http.StatusText(http.StatusPartialContent))
		return
	...
	if len(gdata.Ingredients) <= 0 {
		utils.Dumper("RECORD_NOT_FOUND::ALREADY_EMPTY", gdata.Name, prodID)
		//400
		api.ReplyErrContent(w, r, http.StatusNotFound, "Ingredients already deleted")
		return
	}
	...
	render.JSON(w, r, APIResponse{
		Code:   http.StatusOK,
		Status: "Delete Ingredients successful",
	})
}
